...
8. CAMPO `alert`: contiene voci già strutturate (dati verificati). Regole per voce:
   ...
   b) Voci "Rischi segnalati dalla fonte (testo originale in inglese, non tradotto per non alterarne il significato): ...":
      - se il contenuto è esattamente "None known" (o equivalente banale), sostituiscila con "Rischi segnalati dalla fonte: nessun rischio noto";
      - ALTRIMENTI (qualsiasi contenuto sostanzioso: tossicità, commestibilità, uso medicinale, costituenti chimici, irritazione, allergeni, ...) restituiscila ESATTAMENTE com'è, dicitura inclusa. NON tradurla e NON portarne il contenuto in `descrizione`: è rimandata a un giro di traduzione dedicato. Questa omissione NON viola la regola 1 (è un rinvio consapevole, non una perdita).
   c) Le altre voci (nome comune inglese, rusticità con codice, dimensioni RHS, disclaimer "scheda in bozza") vanno restituite ESATTAMENTE come sono.
   Restituisci l'intero array `alert`, nello stesso ordine e con lo stesso numero di voci dell'originale, con solo le voci pertinenti modificate.
