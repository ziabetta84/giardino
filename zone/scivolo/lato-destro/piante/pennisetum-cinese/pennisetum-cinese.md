---
nome: pennisetum-cinese
specie: Pennisetum alopecuroides 'Hameln'
famiglia: Poaceae
zona: scrivolo
sottozona: lato-destro
attivita:
  irrigazione:
    primavera: ogni 7-10 giorni se non piove
    estate: ogni 5-7 giorni nei periodi caldi
    autunno: ogni 10-14 giorni
    inverno: nessuna, salvo siccità prolungata
  concimazione:
    primavera: una volta con concime leggero per graminacee
    estate: facoltativa una concimazione mensile
    autunno: sospesa
    inverno: sospesa
  potatura:
    primavera: taglio post-fioritura
    estate: nessuna
    autunno: taglio leggero
    inverno: nessuna
alert:
- cultivar compatta (60–80 cm) → ideale per bordure e aiuole strette
- richiede terreno drenante (evitare ristagni)
- taglio annuale indispensabile a fine inverno
- in ombra fitta produce poche spighe
- molto resistente una volta stabilito
ultimo_controllo: 2026-04-17
---

