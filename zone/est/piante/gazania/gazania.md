---
nome: gazania
specie: Gazania rigens
zona: est
sottozona: nessuna
attivita:
  irrigazione:
    primavera: ogni 7-10 giorni
    estate: ogni 5-7 giorni nei periodi molto caldi
    autunno: ogni 10-14 giorni
    inverno: sospesa (solo se in vaso e completamente asciutta)
  concimazione:
    primavera: ogni 4-6 settimane con concime per fiorite
    estate: ogni 4-6 settimane
    autunno: sospesa
    inverno: sospesa
  potatura:
    primavera: taglio leggero
    estate: taglio leggero
    autunno: nessuna
    inverno: nessuna
alert:
- richiede pieno sole per aprire i fiori
- evitare ristagni idrici (ama terreni sabbiosi e drenanti)
- fiori che non si aprono → poca luce
- foglie molli → troppa acqua
- ottima resistenza alla siccità
ultimo_controllo: 2026-04-17
---

